...
from src.exploration_policy.type_aliases_MOD_DISC import DictRolloutBufferSamples
...
class RolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        #self.aux = np.zeros((self.buffer_size, self.n_envs, 2), dtype=np.float32)
...
class DictRolloutBuffer(RolloutBuffer):
    ...
    def __init__(
        ...
        self.use_aux = use_aux#Fabi
        self.aux_dim = aux_dim
        self.gae_lambda = gae_lambda
        self.gamma = gamma
        #added by Fabi
        if(self.use_aux):
            self.observations, self.actions, self.rewards, self.advantages,self.aux_angle,self.aux_angle_gt = None, None, None, None, None, None
        else:
            self.observations, self.actions, self.rewards, self.advantages = None, None, None, None
    ...
    #COMPLETE METHOD ADDED FOR AUXILLIARY FABI
    def add2(
        self,
        obs: Dict[str, np.ndarray],
        action: np.ndarray,
        ...
        value: th.Tensor,
        log_prob: th.Tensor,
        aux_angle: np.ndarray,#added by Fabi
        aux_angle_gt: np.ndarray,#added by Fabi
    ) -> None:
        """
        :param obs: Observation
        :param action: Action
        ...
        #print("current aux's {},{},{}".format(aux,true_aux,aux.shape))
        #print("WHOLE:",self.aux)
        self.pos += 1

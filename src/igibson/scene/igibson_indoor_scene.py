...
class InteractiveIndoorScene(StaticIndoorScene):
    ...
    def __init__(
        ...
        for link in self.scene_tree.findall("link"):
            ...
            # Non-robot object
                ...
                if object_name.startswith("door"):
                    #door stuff
                    kwargs = {'class_id':door_id_counter}
                    obj = URDFObject(
                        filename,
                        name=object_name,
                        category=category,
            ...
            if "multiplexer" in link.keys() or "grouper" in link.keys():
                ...
                self.add_object(obj, simulator=None)
                if object_name.startswith("door"):
                    self.door_list[object_name]= [obj,door_id_counter-1]
    ...
    def remove_or_add_node(self,floor,world_xy,remove=False):
        map_xy = tuple(self.world_to_map(world_xy[:2]))
        g = self.floor_graph[floor]

        if remove:
            if g.has_node(map_xy):
                print("removed node !")
                g.remove_node(map_xy)
                return True
            else:
                print("not in graph yet")
                nodes = np.array(g.nodes)
                closest_node = tuple(nodes[np.argmin(np.linalg.norm(nodes - map_xy, axis=1))])

            
                print("clostest node gets removed")
                g.remove_node(closest_node)
                return False, closest_node
        else:
            if g.has_node(map_xy):
                print("graph already has node")
            else:

                print("Door node gets added")
                #add node since, its not in graph and wants to be added
                g.add_node(map_xy)
                # 8-connected graph
                neighbors = [(map_xy[0] - 1, map_xy[1] - 1), (map_xy[0], map_xy[1] - 1), (map_xy[0] + 1, map_xy[1] - 1), (map_xy[0] - 1, map_xy[1])]
                for n in neighbors:
                    if 0 <= n[0] < self.trav_map_size and 0 <= n[1] < self.trav_map_size and self.floor_map[floor][n[0], n[1]] > 0: #self.floor_map is equal to trav_map
                        g.add_edge(n, map_xy, weight=l2_distance(n, map_xy))
    ...
    def open_one_obj(self, body_id, mode="random"):
        ...
        for joint_id in range(p.getNumJoints(body_id)):
            ...
            p.removeState(state_id)
        if mode == "max":
            return body_joint_pairs,iteration
